...
export const Login: React.FC = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      let success = await login('', email, '');

      if (!success) {
        const localProfiles = dbManager.getProfiles();
        const existing = localProfiles.find(p => p.email && p.email.toLowerCase() === email);

        if (existing && existing.businessId) {
          dbManager.setActiveUser(existing.id);
          dbManager.setActiveBusiness(existing.businessId);
          SessionManager.setToken('google_firebase_' + user.uid);
          window.dispatchEvent(new Event('storage'));
          window.dispatchEvent(new Event('apex-db-update'));
          setSuccessMessage('Google Authentication Verified. Opening Workspace...');
          return;
        }

        const bizName = user.displayName ? `${user.displayName}'s Workspace` : 'Google Corporate Workspace';
        const regSuccess = await registerTenant(
          bizName,
        ...
        if (regSuccess) {
          setSuccessMessage('Google Account Verified & Tenant Provisioned!');
        } else {
          throw new Error('Could not provision workspace for Google account.');
        }
      } else {
        setSuccessMessage('Google Authentication Verified. Opening Workspace...');
      }
                ...
                <form onSubmit={handleRegister} className="space-y-5">
                  ...
                  <div className="relative my-3">
                    ...
                    <div className="relative flex justify-center text-[10px] uppercase font-mono">
                      <span className="bg-gray-900/90 px-2 text-gray-400">Or Quick Register With</span>
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    disabled={isSubmitting}
                    className="w-full flex items-center justify-center gap-2.5 bg-gray-950 hover:bg-gray-900 text-gray-200 border border-gray-800 hover:border-cyan-500/40 font-sans font-medium text-xs py-3 px-4 rounded-xl shadow-sm transition duration-200 cursor-pointer disabled:opacity-50"
                  >
                    ...
                    <span>Register with Google (Firebase)</span>
                  </button>
                </form>
                ...
                {loginMode === 'employee' ? (
                  ...
                  <form onSubmit={handleFormLogin} className="space-y-4">
                    ...
                    <div className="relative my-3">
                      ...
                      <div className="relative flex justify-center text-[10px] uppercase font-mono">
                        <span className="bg-gray-900/90 px-2 text-gray-400">Or Continue With</span>
                      </div>
                    </div>
